...
const syncCooldownMs = Number(appConfig.syncCooldownMs) || 5000;
...
async function syncPendingDeletes() {
    const pendingDeletes = database.getPendingDeletes();
    ...
    database.setPendingDeletes(failed);
    return { ok: failed.length === 0 };
}

async function syncPendingProducts() {
    const pendingProducts = database.getUnsyncedProducts();
    ...
    for (const product of pendingProducts) {
        ...
        if (result.ok && result.data && result.data.success) {
            await database.saveProduct(result.data.product || product, { markSynced: true });
        } else {
            failed += 1;
        }
    }
...
async function syncPendingChanges() {
    const now = Date.now();
    if (now - lastSyncAt < syncCooldownMs) {
        return { ok: true, skipped: true };
    }

    lastSyncAt = now;
    const deleteResult = await syncPendingDeletes();
    const productResult = await syncPendingProducts();
    return { ok: deleteResult.ok && productResult.ok };
}
...
ipcMain.handle('get-products', async () => {
    ...
    if (isOnline) {
        ...
        if (result.ok && Array.isArray(result.data)) {
            if (syncResult.ok) {
                await database.setProducts(result.data, { markSynced: true });
            }
            return result.data;
        }
    }
...
ipcMain.handle('delete-product', async (event, id) => {
    ...
    if (isOnline) {
        ...
        if (result.ok && result.data && result.data.success) {
            ...
            database.removePendingDeletes([id]);
            return result.data;
        }
    }

    const localResult = await database.deleteProduct(id);
    database.queueDelete(id);
    return localResult;
});
